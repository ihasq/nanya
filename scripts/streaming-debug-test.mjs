...
const SCREENSHOT_DIR = '/home/ihasq/code/nanya/debug-screenshots';
const LOCAL_URL = 'http://localhost:5173/test/streaming-debug';
const TICK_MS = 500;
...
async function main() {
  ...
  try {
    ...
    console.log('\n=== TEST 3: Third turn (verify pattern) ===');

    // Click "Start Third Turn"
    await page.click('#start-third-turn');

    // Capture frames during streaming
    for (let i = 0; i < 10; i++) {
      await sleep(TICK_MS);
      await captureFrame(page, `06-third-turn-${String(i).padStart(2, '0')}`);
    }

    await sleep(1000);
    await captureFrame(page, '07-third-turn-complete');
